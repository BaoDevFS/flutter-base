...
class REEToast extends StatefulWidget {
  final Widget child;

  const REEToast({required this.child});

  @override
  REEToastState createState() {
    return REEToastState();
  }

  static REEToastState of(BuildContext context) {
    final REEToastState toastState = context.findRootAncestorStateOfType<REEToastState>()!;
    toastState._setCurrentContext(context);
    return toastState;
  }
}

class REEToastState extends State<REEToast> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    offset = Tween<Offset>(begin: Offset.zero, end: const Offset(0.0, 0.1)).animate(controller);
  }
  ...
  Future showToast({required Color color, String? message, Widget? child, Duration? duration, VoidCallback? onDismissed}) async {
    ...
    await _dismissToastAfterDelay(duration ?? const Duration(seconds: 2));
  }
